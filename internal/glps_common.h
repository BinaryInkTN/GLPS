...
typedef struct
{
  ...
  // Thread management
  void *thread;       /**< Thread for audio playback. */
  void *volume_mutex; /**< Mutex for volume control. */
  void *state_mutex;  /**< Mutex for state control. */

  const char *audio_file_path; /**< Path to the audio file. */
  const char *device_name;     /**< Name of the audio device. */
  int buffer_frames;           /**< Number of frames in the buffer. */
  float *pcm_buffer;           /**< Pointer to the PCM buffer. */
  ...
  int position;                /**< Position in the audio stream. */
  int is_playing;              /**< Flag indicating if audio is playing. */
  int is_paused;               /**< Flag indicating if audio is paused. */
  int is_stopped;              /**< Flag indicating if audio is stopped. */
  void *pcm_handle;            /**< Pointer to the PCM handle. */
  void *mp3;                   /**< Pointer to the MP3 decoder. */
} glps_audio_stream;
